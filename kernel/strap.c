...
static void handle_syscall(trapframe *tf) {
  ...
  uint64 ret=do_syscall(tf->regs.a0, tf->regs.a1, tf->regs.a2, tf->regs.a3, tf->regs.a4, tf->regs.a5, tf->regs.a6, tf->regs.a7);
  tf->regs.a0=ret;
}
...
void handle_user_page_fault(uint64 mcause, uint64 sepc, uint64 stval) {
  ...
  switch (mcause) {
    case CAUSE_STORE_PAGE_FAULT:
    {
      ...
      {
        void *pa=alloc_page();
        if (pa==0) panic("user men alloc falied\n");
        user_vm_map(current->pagetable, stval, 1, (uint64)pa, prot_to_type(PROT_WRITE|PROT_READ, 1));
      }
